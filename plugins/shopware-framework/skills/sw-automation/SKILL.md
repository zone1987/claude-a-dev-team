...
- **[EVENTS-REFERENCE.md](references/EVENTS-REFERENCE.md)**: Complete table of all webhook events with event name, description, required permissions and payload str….
...
- **[FLOW-REFERENCE.md](references/FLOW-REFERENCE.md)**: Schema for `Resources/flow-action.xml` in apps: `<flow-actions>` → `<flow-action>` with `<meta>`, `<headers>`,….
