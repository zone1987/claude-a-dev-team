...
Restructured from 12 skills into 2 domain skills. **Breaking:** every skill ID changed.

### Why

12 skills cost 7,074 characters of the skill listing budget — 88 % of the 8,000 available at a
...
- **12 skills → 2**, grouped by domain. Listing cost 7,074 → 573 characters
  (88 % → 7 %).
- **No knowledge removed.** Every former `SKILL.md` body became a reference file; all reference
  ...
  (`scripts/verify-bundle.py`). 25 reference files remain.
...
Initial release — 12 skills, one per documentation topic.
