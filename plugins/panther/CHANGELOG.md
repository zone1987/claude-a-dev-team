...
Restructured from 11 skills into 2 domain skills. **Breaking:** every skill ID changed.

### Why

11 skills cost 8,779 characters of the skill listing budget — 110 % of the 8,000 available at a
...
- **11 skills → 2**, grouped by domain. Listing cost 8,779 → 525 characters
  (110 % → 7 %).
- **No knowledge removed.** Every former `SKILL.md` body became a reference file; all reference
  ...
  (`scripts/verify-bundle.py`). 24 reference files remain.
...
Initial release — 11 skills, one per documentation topic.
