...
- **[API-FLOWS.md](references/API-FLOWS.md)**: Complete walkthroughs as bash/curl sequences for local development on `http://127.0.0.1:8000`.
...
- **[API-INTEGRATION.md](references/API-INTEGRATION.md)**: Prefer `POST /api/search/{entity}` over `GET /api/{entity}` — it supports filter/sort/associations.
