...
model: sonnet
skills: gotenberg-operations, gotenberg-convert
---
...
- **Configuration:** take all flags/env vars ONLY from `gotenberg-operations` (api-*, chromium-*, libreoffice-*,
  pdfengines-*, webhook-*, log-*, prometheus-*). Do not guess defaults.
- **Security:** outbound URL filtering against SSRF (`gotenberg-operations`), basic auth optional
...
- **Observability:** Prometheus metrics (`/prometheus/metrics`), debug route (`/debug`), trace header
  (`Gotenberg-Trace`), telemetry (`gotenberg-operations`), log level/format.
- **Async load:** for long jobs use webhook mode instead of synchronous requests (`gotenberg-operations`) + timeout tuning.

## Procedure
1. Clarify the target platform → matching manifest (`gotenberg-operations`).
2. Set the necessary flags/env vars (`gotenberg-operations`); don't forget the health check + resource limits.
3. Problems → `gotenberg-operations` (timeouts, crashes, memory, empty/truncated PDFs).
