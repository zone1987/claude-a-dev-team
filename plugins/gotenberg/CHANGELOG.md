...
Restructured from 27 skills into 3 domain skills. **Breaking:** every skill ID changed.

### Why

27 skills cost 11,502 characters of the skill listing budget — 144 % of the 8,000 available at a
...
- **27 skills → 3**, grouped by domain. Listing cost 11,502 → 892 characters
  (144 % → 11 %).
- **No knowledge removed.** Every former `SKILL.md` body became a reference file; all reference
  ...
  (`scripts/verify-bundle.py`). 54 reference files remain.
...
Initial release — 27 skills, one per documentation topic.
