...
Every numeric code Discord defines: Gateway opcodes, Gateway close event codes, voice opcodes, voice
close event codes, HTTP response codes, the complete JSON error code table, and RPC error and close
codes.
...
- [Gateway close event codes](#gateway-close-event-codes)
...
All gateway events in Discord are tagged with an opcode that denotes the payload type. Your connection
to the gateway may also sometimes close. When it does, you will receive a close code that tells you what
happened.
...
Opcode `5` and the ranges between the listed numbers are not documented upstream.

## Gateway close event codes

In order to prevent broken reconnect loops, you should consider some close codes as a signal to stop
reconnecting. This can be because your token expired, or your identification is invalid. This table
explains what the application defined close codes for the gateway are, and which close codes you should
...
Close code `4006` is not documented for the Gateway. Codes `1000` and `1001` are standard WebSocket
close codes: closing with either invalidates the session and the bot appears offline; any other close
code (or a bare TCP close) leaves the session active for a few minutes so it can be resumed.

## Voice opcodes

Discord's voice gateways have their own set of opcodes and close codes. The "Binary" column marks
opcodes whose payloads are binary.
...
Voice opcodes `10`, `12`, and `14` through `20` are not documented upstream.
...
Voice close codes `4000`, `4007`, `4008`, `4010`, `4013`, `4018`, `4019` are not documented upstream.

## HTTP response codes

Discord's API will return semantically valid HTTP response codes based on the success of your request.
The following table can be used as a reference for response codes it will return.
...
Along with the HTTP error code, Discord's API can also return more detailed error codes through a `code`
key in the JSON error response. The response will also contain a `message` key containing a more
friendly error string. Some of these errors may include additional details in the form of Error Messages
provided by an `errors` object.
...
RPC is the local Discord server running on localhost. Access to the RPC server requires approval from
Discord.
