...
Eigene Registry aufsetzen. Skills: `shadcn-vue-registry`, `shadcn-vue-registry-json`, `shadcn-vue-registry-item-json`,
`shadcn-vue-registry-examples`, bei `--mcp` `shadcn-vue-mcp`.

## Ablauf
1. **`registry.json`** erstellen (name, homepage, items[]) — Schema aus `shadcn-vue-registry-json`.
2. **`registry-item.json`** je Item nach `$ARGUMENTS` — alle nötigen Felder (`name`, `type` registry:<…>, `title`,
   ...
   `meta`, `docs`) — Schema aus `shadcn-vue-registry-item-json`. Nur dokumentierte Felder. Vue: `.vue`-SFC, `composables`.
...
5. `--mcp` → `registry.json` an der Wurzel für MCP-Kompatibilität (`shadcn-vue-mcp`).
