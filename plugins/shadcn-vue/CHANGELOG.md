...
Restructured from 93 skills into 8 domain skills. **Breaking:** every skill ID changed.

### Why

93 skills cost 42,448 characters of the skill listing budget — 531 % of the 8,000 available at a
...
- **93 skills → 8**, grouped by domain. Listing cost 42,448 → 2,282 characters
  (531 % → 29 %).
- **No knowledge removed.** Every former `SKILL.md` body became a reference file; all reference
  ...
  (`scripts/verify-bundle.py`). 400 reference files remain.
...
Initial release — 93 skills, one per documentation topic.
