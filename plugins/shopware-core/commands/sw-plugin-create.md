...
4. **Licence**: `proprietary` — in `composer.json` **and** in a `LICENSE` file written out in
   full. There is no choice to make here.
   → `shopware-devops` → `sw-tooling` → `PLUGIN-COMPLIANCE-ARTEFACTS.md`
...
│   └── Resources/config/services.php   # PHP, not XML; imports one file per area from services/
├── README.md
└── CHANGELOG.md
```
