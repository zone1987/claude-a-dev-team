...
Restructured from 33 skills into 4 domain skills. **Breaking:** every skill ID changed.

### Why

33 skills cost 14,471 characters of the skill listing budget — 181 % of the 8,000 available at a
...
- **33 skills → 4**, grouped by domain. Listing cost 14,471 → 1,146 characters
  (181 % → 14 %).
- **No knowledge removed.** Every former `SKILL.md` body became a reference file; all reference
  ...
  (`scripts/verify-bundle.py`). 92 reference files and 2 bundled files remain.
...
Initial release — 33 skills, one per documentation topic.
