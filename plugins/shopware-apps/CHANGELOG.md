...
Restructured from 5 skills into 2 domain skills. **Breaking:** every skill ID changed.

### Why

5 skills cost 3,012 characters of the skill listing budget — 38 % of the 8,000 available at a
...
- **5 skills → 2**, grouped by domain. Listing cost 3,012 → 584 characters
  (38 % → 7 %).
- **No knowledge removed.** Every former `SKILL.md` body became a reference file; all reference
  ...
  (`scripts/verify-bundle.py`). 76 reference files and 19 bundled files remain.
...
Initial release — 5 skills, one per documentation topic.
