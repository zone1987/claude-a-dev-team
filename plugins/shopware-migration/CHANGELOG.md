...
Restructured from 8 skills into 2 domain skills. **Breaking:** every skill ID changed.

### Why

8 skills cost 3,525 characters of the skill listing budget — 44 % of the 8,000 available at a
...
- **8 skills → 2**, grouped by domain. Listing cost 3,525 → 550 characters
  (44 % → 7 %).
- **No knowledge removed.** Every former `SKILL.md` body became a reference file; all reference
  ...
  (`scripts/verify-bundle.py`). 1061 reference files and 9 bundled files remain.
...
Initial release — 8 skills, one per documentation topic.
