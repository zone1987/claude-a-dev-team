...
- **Tools**: the Rector set of the version the plugin is released for, for deprecated APIs (`shopware-quality` → `sw-rector`);
  `composer gate` after every step — it runs the fixers and every check, and it is the one command before a commit.
