...
Basic item with title, description, and action. Second item rendered as link with icon media and `asChild`.
...
`ItemMedia` with `variant="icon"` displays an icon in a bordered box.
...
`ItemMedia` (default variant) with an `Avatar` component.
...
`ItemMedia` with `variant="image"` clips images to a 40px square.
...
`ItemGroup` with `ItemSeparator` between entries.
...
`ItemHeader` adds a full-width row above the item content.
...
Use `asChild` to render `Item` as an `<a>` tag with hover/focus states.
...
Item used inside a `DropdownMenu` for rich people-picker UI.
...
---
Source: `registry/new-york-v4/examples/item-*.tsx`
